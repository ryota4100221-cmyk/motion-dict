...
export const textOnPath: MotionEntry = {
  ...
  params: [
    ...
    },
    {
      ...
      step: 10,
      default: 180,
      desc: "スクロールを最後まで送ったときに文字列がパス上を進む距離。「文字列の長さ + 100%」がちょうど画面を通り抜ける値で、既定の180%はそこに少し余裕を足したもの。100%を切ると文が入りきる前に止まる。",
    },
    {
      key: "length",
...
- prefers-reduced-motion 時はスクロール連動を止め、text-anchor: middle・startOffset: 50% で文字列を弧の中央に静止表示する（見出しは必ず読める状態で残す）`,
  ...
  okExample: {
    say: "「SVGのtextPathで見出しを2次ベジェに沿わせ、スクロール進捗をstartOffsetの100%→-80%に写像。セクションはstickyでステージ高3倍ぶん固定。更新はstartOffset属性のみ、reduced-motion時は50%で静止」",
    why: "組版はtextPathに任せる・動かすのはstartOffsetだけ、と実装方式を先に固定している。送りの範囲（%）と尺（3倍）が数値で入っているので速度が一意に決まり、静止時の見え方まで指定されている。",
  },
  vocab: [
    {
      term: "textPath",
