...
export const textOnPath: MotionEntry = {
  ...
  params: [
    ...
    },
    {
      ...
      step: 10,
      default: 110,
      desc: "スクロールを最後まで送ったときに文字列がパス上を進む距離。頭出し位置(startOffset 70%)から始めるので、110%で「先頭が右に掛かった状態」から「文末が左へ抜ける寸前」までを通しで見せられる。130%を超えると両端で見出しが完全に画面外に出て、文字の居ない«間»ができる。60%台では弧を渡りきらない。",
    },
    {
      key: "length",
...
- スクロール進捗（0→1）を startOffset の % に写像し、70% から (70 - {{travel}})% まで動かす。単位は必ず%にしてパス長の実測に依存させない
- 開始は 0% ではなく「見出しの先頭が画面端に掛かる位置」(ここでは70%)にする。空の弧だけが固定表示される瞬間を作らないため
...
- prefers-reduced-motion 時はスクロール連動を止め、text-anchor: middle・startOffset: 50% で文字列を弧の中央に静止表示する（見出しは必ず読める状態で残す）`,
  ...
  okExample: {
    say: "「SVGのtextPathで見出しを2次ベジェに沿わせ、スクロール進捗をstartOffsetの70%→-40%に写像。セクションはstickyでステージ高4倍ぶん固定。更新はstartOffset属性のみ、reduced-motion時は50%で静止」",
    why: "組版はtextPathに任せる・動かすのはstartOffsetだけ、と実装方式を先に固定している。送りの範囲（%）と尺（4倍）が数値で入っているので速度が一意に決まる。開始を0%ではなく70%に置いている点も効いていて、ピンに入った瞬間から見出しが読める。",
  },
  vocab: [
    {
      term: "textPath",
