...
export const halftone: MotionEntry = {
  ...
  lede: "絵や文字の濃さを網点(大小の丸)の粒に置き換えて刷り直す表現。カーソルが近づくと網点が太り、色版が左右にズレて印刷の版ズレのように見えるのが動きの核で、静止画のフィルタでは出せない「刷り物の生々しさ」が出る。",
  params: [
    {
      key: "gap",
      label: "gap(網点の間隔 px)",
...
- 効果の中心からの距離 d が {{reach}}px 以内の点は近接度 k = 1 - d / {{reach}} を求め、r に k を上乗せして太らせる
- 効果の中心はカーソルの位置とする。カーソルが乗っていない間は中心を画面内でゆっくり(7秒前後で1周)巡回させ、触らなくても何が起きる演出なのかが伝わるようにする
...
- prefers-reduced-motion 時はカーソル追従も巡回も版ズレも完全に止め、網点化しただけの静止画を1枚描いて終わりにする`,
  ngExample: {
